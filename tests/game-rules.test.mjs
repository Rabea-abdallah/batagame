...
import { CARD_TYPES, DEFAULT_RULES, RULE_PROFILES, GAME_PHASES, VOICE_MAX_CLIPS_PER_PLAYER } from '../assets/js/core/Constants.js';
import { VoiceClipStore } from '../assets/js/network/VoiceClipStore.js';
...
test('voice clip store keeps only last 3 clips per player', () => {
  const store = new VoiceClipStore();
  const mk = (i) => ({
    id: `c${i}`,
    mimeType: 'audio/webm',
    durationMs: 1000,
    createdAt: i,
    audioBase64: btoa('test-audio-chunk')
  });

  for (let i = 1; i <= 4; i++) store.addClip('p1', mk(i));
  const clips = store.getClips('p1');

  assert.equal(clips.length, VOICE_MAX_CLIPS_PER_PLAYER);
  assert.equal(clips[0].id, 'c2');
  assert.equal(clips[2].id, 'c4');
  store.destroy();
});
