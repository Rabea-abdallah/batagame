...
import { VoiceService } from './network/VoiceService.js';
...
class App {
  ...
  _setupGameEvents() {
    ...
    eventBus.on('voice:error', (data) => {
      if (data?.message) Notification.warning(data.message, 9000);
    });
  }

  _initVoiceService() {
    if (!this.roomManager || this.isAIMode) return;
    if (this.voiceService) return;

    const pc = this.roomManager.peerConnection;
    this.voiceService = new VoiceService({
      playerId: this.roomManager.playerId,
      isHost: pc.isHost,
      sendEvent: (event, payload) => pc.sendEvent(event, payload),
      sendEventTo: (peerId, event, payload) => pc.sendEventTo(peerId, event, payload),
      broadcast: (event, payload, excludePeer) => pc.broadcast(event, payload, excludePeer)
    });

    if (!pc.isHost) {
      this.voiceService.requestSync();
    }
  }

  _ensureGameVoiceUI() {
    if (!this.gameUI || !this.voiceService || this.isAIMode) return;
    if (!this.gameUI._voiceProvider) {
      this.gameUI.setVoiceProvider(this.voiceService);
    } else {
      this.gameUI._updateVoiceUI();
    }
  }

  _setupVoiceNetworkEvents() {
    const onVoiceMessage = (data) => {
      if (!this.voiceService || this.isAIMode) return;
      const pc = this.roomManager?.peerConnection;
      if (!pc || !data?.playerId) return;

      // Ignore our own uploads echoed back (guest relay).
      if (data.playerId === this.voiceService.playerId && data._fromPeer === pc.myId) return;

      if (pc.isHost) {
        const fromGuest = data._fromPeer && data._fromPeer === data.playerId;
        if (data._fromPeer && !fromGuest) return;
        this.voiceService.handleIncomingVoice(data, {
          fromPeer: data._fromPeer || null,
          relay: !!fromGuest
        });
      } else {
        this.voiceService.handleIncomingVoice(data, { relay: false });
      }

      this._ensureGameVoiceUI();
    };

    eventBus.on(EVENTS.VOICE_CLIP, onVoiceMessage);
    eventBus.on(EVENTS.VOICE_CLIP_PART, onVoiceMessage);

    eventBus.on(EVENTS.VOICE_SYNC_REQUEST, (data) => {
      if (!this.voiceService || !this.roomManager?.peerConnection?.isHost || !data._fromPeer) return;
      this.voiceService.sendFullSyncTo(data._fromPeer);
    });

    eventBus.on(EVENTS.VOICE_FULL_SYNC, (data) => {
      if (!this.voiceService || this.roomManager?.peerConnection?.isHost) return;
      this.voiceService.handleFullSync(data);
      this._ensureGameVoiceUI();
    });
  }
  ...
  _showGame() {
    ...
    if (this.voiceService) this.gameUI.setVoiceProvider(this.voiceService);
    if (this.voiceService && !this.isAIMode && !this.roomManager?.peerConnection?.isHost) {
      this.voiceService.requestSync();
    }
  ...
  _onPlayerDisconnect(peerId) {
    ...
    this.voiceService?.onPlayerLeft(peerId);
  ...
  _cleanup() {
    ...
    if (this.voiceService) { this.voiceService.destroy(); this.voiceService = null; }
