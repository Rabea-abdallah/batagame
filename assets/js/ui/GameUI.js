...
import { beginMicrophoneCapture } from '../network/MicrophoneAccess.js';
...
export class GameUI {
    ...
    this._container.innerHTML = `
      <div class="game">
        ...
        <div class="game__board">
          ...
          <div class="game__voice-local" id="voice-local" hidden aria-label="تسجيلات صوتية">
            <button type="button" class="voice-btn voice-btn--record" id="btn-voice-record" title="تسجيل صوتي">
              <span class="voice-btn__icon">🎤</span>
              <span class="voice-btn__label">تسجيل</span>
            </button>
            <div class="voice-clips" id="voice-local-clips"></div>
          </div>
  ...
  setVoiceProvider(provider) {
    this._voiceProvider = provider || null;
    this._updateVoiceUI();
  }
  ...
  _setupVoiceListeners() {
    ...
    const localBar = document.getElementById('voice-local');
    if (localBar) localBar.hidden = false;

    document.getElementById('btn-voice-record')?.addEventListener('click', () => {
      if (this._isAIMode) return;
      if (!this._voiceProvider) {
        Notification.warning('التسجيل الصوتي متاح في الروم فقط.');
        return;
      }
      if (this._voiceProvider.isRecording()) {
        void this._voiceProvider.finishRecording();
      } else {
        void this._voiceProvider.startRecordingFromCapture(beginMicrophoneCapture());
      }
    });

    this._unsubscribers.push(
      eventBus.on('voice:clips-updated', () => this._updateVoiceUI())
    );
    this._unsubscribers.push(
      eventBus.on('voice:recording-state', (data) => {
        this._voiceRecording = !!data?.recording;
        this._patchRecordButton();
      })
    );
    this._unsubscribers.push(
      eventBus.on('voice:playback-state', (data) => {
        this._voicePlayingKey = data?.playing ? `${data.playerId}:${data.clipId}` : null;
        this._updateVoiceUI();
      })
    );
  }
  ...
  _patchRecordButton() {
    ...
    if (!btn) return;
    btn.classList.toggle('voice-btn--recording', this._voiceRecording);
    const label = btn.querySelector('.voice-btn__label');
    const icon = btn.querySelector('.voice-btn__icon');
    if (label) label.textContent = this._voiceRecording ? 'إيقاف' : 'تسجيل';
    if (icon) icon.textContent = this._voiceRecording ? '⏹️' : '🎤';
  }
  ...
  _renderVoiceClipButtons(container, playerId, clips) {
    if (!container) return;
    ...
    clips.forEach((clip, index) => {
      ...
      btn.type = 'button';
      btn.className = 'voice-btn voice-btn--clip';
      btn.dataset.playerId = playerId;
      btn.dataset.clipId = clip.id;
      btn.title = `تشغيل التسجيل ${index + 1}`;
      const playing = this._voicePlayingKey === `${playerId}:${clip.id}`;
      btn.classList.toggle('voice-btn--playing', playing);
      btn.innerHTML = `<span class="voice-btn__icon">${playing ? '⏸' : '▶'}</span><span class="voice-btn__num">${index + 1}</span>`;
      btn.addEventListener('click', (e) => {
        e.stopPropagation();
        eventBus.emit('voice:play', { playerId, clipId: clip.id });
      });
      container.appendChild(btn);
    });
  }

  _patchOpponentVoice(opponentEl, playerId) {
    if (!opponentEl || this._isAIMode) return;

    let row = opponentEl.querySelector('.opponent__voice');
    if (!row) {
      row = document.createElement('div');
      row.className = 'opponent__voice voice-clips';
      row.setAttribute('aria-label', 'تسجيلات صوتية');
      const info = opponentEl.querySelector('.opponent__info');
      if (info) info.appendChild(row);
      else {
        const cardsRow = opponentEl.querySelector('.opponent__cards-row');
        if (cardsRow) opponentEl.insertBefore(row, cardsRow);
        else opponentEl.appendChild(row);
      }
    }

    if (!this._voiceProvider) {
      row.hidden = true;
      opponentEl.classList.remove('opponent--has-voice');
      return;
    }

    const clips = this._voiceProvider.getClips(playerId);
    opponentEl.classList.toggle('opponent--has-voice', clips.length > 0);
    this._renderVoiceClipButtons(row, playerId, clips);
  }
  ...
  _updateVoiceUI() {
    ...
    if (this._voiceProvider) {
      const localClips = this._voiceProvider.getClips(this._playerId);
      this._renderVoiceClipButtons(document.getElementById('voice-local-clips'), this._playerId, localClips);
      this._patchRecordButton();
    }

    const opponents = this._getOpponentOrder();
    for (const p of opponents) {
      const el = document.querySelector(`.opponent[data-player-id="${p.id}"]`);
      if (el) this._patchOpponentVoice(el, p.id);
    }
  }
  ...
  _updatePlayers() {
    ...
    opponentOrder.forEach((p, i) => {
      ...
      this._patchOpponentVoice(el, p.id);
  ...
  _cleanup() {
    eventBus.emit('voice:stop-playback');
    this._voiceProvider = null;
    this._voiceRecording = false;
    this._voicePlayingKey = null;
